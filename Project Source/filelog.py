...
def TryLoadPrevTrainingLog(train_log_file, prev_train_loss, prev_test_acc):
...
def Train_LogDetails_Update(train_log_file, batch, mean_inbatch, std_inbatch, mean_outbatch, std_outbatch):
    if train_log_file == '':
        return
    file_detail.write('Batch = {:03d}'.format(batch) + '  MeanIn = {:6.3f}%'.format(mean_inbatch*100) + '  StdIn = {:6.3f}%'.format(std_inbatch*100) + '  MeanOut = {:6.3f}%'.format(mean_outbatch*100) + '  StdOut = {:6.3f}%\n'.format(std_outbatch*100))
    file_detail.flush()
